  ...
  function scriptBuildUI(thisObj) {
    var win =
      thisObj instanceof Panel
        ? thisObj
        : new Window("window", "Planning creation assistant", undefined, {
            resizeable: false,
            closeButton: false,
          });
    ...
    function BuildDlg(textLayer1, textLayer2) {
      ...
      var group3 = textDlg.add("group", undefined, "dlgSettings");
      var buttonOk = group3.add("button", undefined, "Ok");
      var buttonCancel = group.add("button", undefined, "Annuler");
      ...
      const prop1 = textLayer1.property("Source Text"),
        prop2 = textLayer2.property("Source Text");
      const propSave = [prop1.value, prop2.value];
      ...
      function TextSize(arg) {
        ...
        arg
          ? (textDocument.fontSize = acutalSize + 1)
          : (textDocument.fontSize = acutalSize - 1);
      ...
      function RepositionAnchorPoint(layer) {
        ...
        layer.name == "Texte personnalisé 1"
          ? layer.position.setValue([400, 57])
          : layer.position.setValue([layerPosition[0] + xAdd, 57]);
      }

      function Cancel() {
        prop1.setValue(propSave[0]);
        prop2.setValue(propSave[1]);
      }
      ...
      buttonCancel.addEventListener("mousedown", Cancel());
      textDlg.show();
    }
    ...
    var buttonActivateLundi = activationPanel.add(
      "checkbox",
      undefined,
      "Lundi",
      {
    ...
    var buttonActivateMardi = activationPanel.add(
      "checkbox",
      undefined,
      "Mardi",
      {
    ...
    var buttonActivateMercredi = activationPanel.add(
      "checkbox",
      undefined,
      "Mercredi",
      {
    ...
    var buttonActivateJeudi = activationPanel.add(
      "checkbox",
      undefined,
      "Jeudi",
      {
    ...
    var buttonActivateVendredi = activationPanel.add(
      "checkbox",
      undefined,
      "Vendredi",
      {
    ...
    var buttonActivateSamedi = activationPanel.add(
      "checkbox",
      undefined,
      "Samedi",
      {
    ...
    var buttonActivateDimanche = activationPanel.add(
      "checkbox",
      undefined,
      "Dimanche",
      {
    ...
    function CreateTab(day) {
      return (
        "tab { alignChildren: 'top', text: '" +
        day.slice(4) +
        "',  \
                    panelSettings: Panel { orientation: 'column', text: 'Layout', \
                         buttonOneSpace: Button {text: 'One space', name: '" +
        day +
        "'}, \
                         buttonTwoSpace: Button {text: 'Two space', name: '" +
        day +
        "'} \
                         buttonOpen: Button {text: 'Ouvrir la comp', name: '" +
        day +
        "'} \
                    }, \
                    groupPanels: Group { orientation: 'column', \
                         panelOne: Panel { alignChildren: 'left', text: 'Première case', \
                              groupHours: Group { orientation: 'row', \
                                   ddHour: DropDownList { title: 'Heure :', name: '" +
        day +
        "' }, \
                                   checkboxFifteen: Checkbox { text: '15', name: '" +
        day +
        "' }, \
                                   checkboxThirty: Checkbox { text: '30', name: '" +
        day +
        "' }, \
                                   checkboxFortyfive: Checkbox { text: '45', name: '" +
        day +
        "' } \
                              }, \
                              ddCategorie: DropDownList { title: 'Catégorie :', name: '" +
        day +
        "' } \
                         }, \
    ...
    for (var i = 0; i < tabsObject.length; i++) {
      ...
      for (var j = 0; j < hours.length; j++) {
        tabsObject[i].groupPanels.panelOne.groupHours.ddHour.add(
          ItemType(hours[j]),
          hours[j]
        );
        tabsObject[i].groupPanels.panelTwo.groupHours.ddHour.add(
          ItemType(hours[j]),
          hours[j]
        );
      }
      ...
      //Open comp
      tabsObject[i].panelSettings.buttonOpen.addEventListener(
        "mousedown",
        function (e) {
      ...
      // Toggle spaces
      tabsObject[i].panelSettings.buttonOneSpace.addEventListener(
        "mousedown",
        function (e) {
          ChangeLayout(true, e.target.name);
        }
      );
      tabsObject[i].panelSettings.buttonTwoSpace.addEventListener(
        "mousedown",
        function (e) {
      ...
      tabsObject[
        i
      ].groupPanels.panelOne.groupHours.checkboxFifteen.addEventListener(
        "mousedown",
        function (e) {
      ...
      tabsObject[
        i
      ].groupPanels.panelOne.groupHours.checkboxThirty.addEventListener(
        "mousedown",
        function (e) {
      ...
      tabsObject[
        i
      ].groupPanels.panelOne.groupHours.checkboxFortyfive.addEventListener(
        "mousedown",
        function (e) {
      ...
      tabsObject[
        i
      ].groupPanels.panelTwo.groupHours.checkboxFifteen.addEventListener(
        "mousedown",
        function (e) {
          ActivateQuarter(e.target, 15);
        }
      ...
      tabsObject[
        i
      ].groupPanels.panelTwo.groupHours.checkboxThirty.addEventListener(
        "mousedown",
        function (e) {
          ActivateQuarter(e.target, 30);
        }
      ...
      tabsObject[
        i
      ].groupPanels.panelTwo.groupHours.checkboxFortyfive.addEventListener(
        "mousedown",
        function (e) {
          ActivateQuarter(e.target, 45);
        }
      ...
      // Dropdowns
      tabsObject[i].groupPanels.panelOne.groupHours.ddHour.addEventListener(
        "change",
        function (e) {
          SetHour(e.target, parseInt(e.target.name.slice(0, 1)));
        }
      );
      tabsObject[i].groupPanels.panelTwo.groupHours.ddHour.addEventListener(
        "change",
        function (e) {
          SetHour(e.target, parseInt(e.target.name.slice(0, 1)) + 7);
        }
      );
      tabsObject[i].groupPanels.panelOne.ddCategorie.addEventListener(
        "change",
        function (e) {
          SetCategorie(e.target, e.target.selection, e.target.name);
        }
      );
      tabsObject[i].groupPanels.panelTwo.ddCategorie.addEventListener(
        "change",
        function (e) {
          SetCategorie(e.target, e.target.selection, e.target.name);
        }
    ...
    function SetCategorie(dd, selection, compName) {
      ...
      const layer2 = GetCompByName(compName + " - Animated").layers.byName(
        selection
      );
      if (
        selection.toString() == "Texte personnalisé" ||
        selection.toString() == "Texte personnalisé 2" ||
        selection.toString() == "Texte personnalisé 3"
      ) {
    ...
    function SetHour(dd, dayNumber) {
      ...
      dd.selection
        ? precompHours
            .layer(dayNumber)
            .text.sourceText.setValue(dd.selection.toString())
        : precompHours.layer(dayNumber).text.sourceText.setValue("Heure");
    }
    ...
    function ActivateQuarter(button, type) {
      ...
      !button.value
        ? precompHours.layers
            .byName("Heure - " + button.name.slice(4) + " - " + type)
    ...
    function ChangeLayout(state, day) {
      ...
      const precompHours = GetCompByName("hours_precomp");
      state
        ? (GetTabByName(day.slice(4)).groupPanels.panelTwo.enabled = false)
        : (GetTabByName(day.slice(4)).groupPanels.panelTwo.enabled = true);
      state
        ? precompShapes.layer(parseInt(day.slice(0, 1))).opacity.setValue(100)
        : precompShapes.layer(parseInt(day.slice(0, 1))).opacity.setValue(0);
      state
        ? precompHours.layer(parseInt(day.slice(0, 1)) + 7).opacity.setValue(0)
        : precompHours
            .layer(parseInt(day.slice(0, 1)) + 7)
            .opacity.setValue(100);
      state
        ? activeDays.push(parseInt(day + 7))
        : activeDays.remove(parseInt(day + 7));
    }
    ...
    function Export() {
      app.project.renderQueue.items
        .add(GetCompByName("PLANNING"))
        .outputModule(1)
        .applyTemplate("PNG");
      app.project.renderQueue.items
        .add(GetCompByName("PLANNING - Animated"))
        .outputModule(1)
        .applyTemplate("ALPHA");
    }
    ...
    function SetDate(dd1, dd2) {
      ...
      layer.property("Source Text").expression =
        "text.sourceText.style.setFontSize(70)";
      if (day >= 25) {
        layer.property("Source Text").expression =
          "text.sourceText.style.setFontSize(53)";
        if (
          dd2.selection.toString() == "janvier" ||
          dd2.selection.toString() == "mars" ||
          dd2.selection.toString() == "mai" ||
          dd2.selection.toString() == "juillet" ||
          dd2.selection.toString() == "août" ||
          dd2.selection.toString() == "octobre" ||
          dd2.selection.toString() == "décembre"
        ) {
      ...
      compPlanning
        .layer(8)
        .text.sourceText.setValue(
          "Planning du " + day + " au " + day2 + " " + month.toString()
        );
    }

    function TimeRemap(layersCollection) {
      for (i in layersCollection) {
        layersCollection[i].expression = null;
      }
      for (n in activeDays) {
        var count = 0;
        var delay = 9 * count;
        count++;
        layersCollection[activeDays[n]].expression =
    ...
    buttonExport.onClick = function () {
      TimeRemap(GetCompByName("PLANNING - Animated").layers);
      Export();
      app.project.save();
    };
